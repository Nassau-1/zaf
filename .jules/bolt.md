...
## 2024-06-25 - Avoid O(N^2) array allocations in recursive traversals
**Learning:** Using `results.concat()` inside a recursive directory traversal function (like `walkDir`) creates O(N²) array allocations and severe memory churn, degrading performance on large directory structures.
**Action:** Always pass a single `results` array by reference through the recursive calls rather than creating and concatenating new arrays at each level to maintain O(N) time and space complexity.
