...
## 2024-06-25 - Avoid array allocations in recursive traversal
**Learning:** Using `array.concat()` in recursive file system traversals like `walkDir` creates O(N²) allocations, adding overhead for large repository structures (e.g. codebase context mapping).
**Action:** Pass a `results` array by reference throughout the recursive function to maintain O(N) performance.
