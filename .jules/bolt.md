...
## 2025-02-19 - Optimizing Recursive Directory Walks
**Learning:** Using `results.concat()` inside recursive directory traversals like `walkDir` creates `O(N^2)` array allocations and memory churn, severely impacting performance for deep/large directories.
**Action:** Always pass a `results` array by reference throughout the recursion (e.g., `function walkDir(dir, maxDepth, depth = 0, results = [])`) to maintain `O(N)` time and space complexity.
