...
## 2024-06-25 - Prevent event loop blocking on expanding array uniqueness lookups
**Learning:** Using `Array.prototype.find()` on an expanding array within a processing loop (like candidate generation for process logs) results in O(N²) time complexity and blocks the Node.js event loop for large sequences.
**Action:** Maintain a parallel `Set` for O(1) uniqueness lookups instead of using `Array.prototype.find()` on expanding arrays when processing large arrays or event sequences.
