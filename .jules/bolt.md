...
## 2024-05-24 - O(N^2) Array.find on large datasets
**Learning:** Using `Array.prototype.find()` within loops on large event sequences causes O(N^2) behavior, blocking the Node event loop and timing out APIs.
**Action:** Always maintain a parallel `Set` for O(1) uniqueness lookups instead of repeatedly searching growing arrays with `find()`.
