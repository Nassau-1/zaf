...
## 2024-07-05 - O(N²) array find in sequence generation
**Learning:** Using `Array.prototype.find()` on an expanding array within a nested iteration loop in a Node.js API endpoint causes O(N²) event-loop blocking execution times for large arrays.
**Action:** Use a parallel `Set` or `Map` to maintain O(1) lookups for uniqueness checks when expanding arrays in loops.
