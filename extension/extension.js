...
async function updateDecorations(editor, context) {
  ...
  try {
    const allFiles = await fs.promises.readdir(activeDir);
    const ticketFiles = allFiles.filter(f => f.endsWith('.md'));
  ...
  context.subscriptions.push(
    ...
    vscode.commands.registerCommand('zaf.launchAgent', async () => {
      ...
      try {
        ...
        await Promise.all(ticketFiles.map(async (tf) => {
          let content;
          try { content = await fs.promises.readFile(path.join(activeDir, tf), 'utf8'); } catch { return; }
          const refs = extractFileRefsFromTicket(content);
