...
// Security Enhancement: Safely resolve paths to prevent path traversal
...
function composeSeedPrompt(opts, ticketBody) {
  ...
  const repoRoot = repoName ? getSafeRepoRoot(repoName) : null;
  ...
  let skillsSection = '';
  if (repoName) {
    const repoRoot = getSafeRepoRoot(repoName);
    if (!repoRoot) return '';
...
function spawnAgent(opts) {
  ...
  const repoRoot = getSafeRepoRoot(repoSlug);
  if (!repoRoot) return { processId: null, message: 'Invalid repo path' };
...
const server = http.createServer(async (req, res) => {
  ...
  if (pathname === '/api/run') {
    const launch = async () => {
      ...
      if (!meta.processId) return send(res, 400, { error: meta.message || 'spawn failed' });
  ...
  if (terminateMatch && req.method === 'POST') {
    ...
    try {
      ...
      const today = new Date().toISOString().slice(0, 10);
      const safeRepo = getSafeRepoRoot(entry.meta.repoId || 'zaf');
      if (!safeRepo) return send(res, 400, { error: 'Invalid repo path' });
      const ticketPath = path.join(safeRepo, 'WIP', 'tickets', 'ACTIVE', `${entry.meta.ticketId}.md`);
      try {
  ...
  if (ticketStatusMatch && req.method === 'POST') {
    try {
      ...
      const safeRepo = getSafeRepoRoot(repoSlug);
      if (!safeRepo) return send(res, 400, { error: 'Invalid repo path' });
      const activeDir = path.join(safeRepo, 'WIP', 'tickets', 'ACTIVE');
      ...
      const indexFile = path.join(safeRepo, 'WIP', 'tickets', 'TICKETS.md');
  ...
  if (ticketArchiveMatch && req.method === 'POST') {
    try {
      ...
      if (!safeRepo) return send(res, 400, { error: 'Invalid repo path' });
      const activeDir = path.join(safeRepo, 'WIP', 'tickets', 'ACTIVE');
      const archivedDir = path.join(safeRepo, 'WIP', 'tickets', 'ARCHIVED');
      const ticketPath = path.join(activeDir, `${ticketId}.md`);
      if (!fs.existsSync(ticketPath)) return send(res, 404, { error: 'ticket not found in ACTIVE/' });
...
`;
  ...
  if (pathname === '/api/repo/skill/update' && req.method === 'POST') {
    try {
      ...
      const repoRoot = getSafeRepoRoot(repo);
      if (!repoRoot) return send(res, 400, { error: 'Invalid repo path' });
