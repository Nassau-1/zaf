...
function resolveSafeRepo(userInput) {
  if (!userInput) return null;
  const resolvedPath = path.resolve(REPOS_ROOT, userInput);
...
function composeSeedPrompt(opts, ticketBody) {
  ...
  const repoRoot = repoName ? resolveSafeRepo(repoName) : null;
  ...
  let skillsSection = '';
  if (repoName) {
    const repoRoot = resolveSafeRepo(repoName);
    if (!repoRoot) return;
...
function spawnAgent(opts) {
  ...
  const repoRoot = resolveSafeRepo(repoSlug);
  if (!repoRoot) { console.error('Invalid repo path'); return; }
...
`;
  ...
  if (pathname === '/api/repo/skill/update' && req.method === 'POST') {
    try {
      ...
      const repoRoot = resolveSafeRepo(repo);
      if (!repoRoot) return send(res, 400, { error: 'Invalid repo path' });
