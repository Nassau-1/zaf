...
`;
  ...
  // ── Static files ───────────────────────────────────────────────────────────
  if (pathname.indexOf('\0') !== -1) { res.writeHead(400); res.end('Bad Request'); return; }
  let safePathname;
  try { safePathname = decodeURIComponent(pathname); } catch (e) { res.writeHead(400); res.end('Bad Request'); return; }
  let filePath = path.join(STATIC_DIR, safePathname === '/' ? 'index.html' : safePathname);
