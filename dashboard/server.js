...
function getSafeRepoPath(repoName) {
...
`;
  ...
  if (pathname === '/api/repo/skills' && req.method === 'GET') {
    ...
    const skillsDir = path.join(getSafeRepoPath(repoSlug), '.zaf-skills');
    try {
  ...
  if (pathname === '/api/repo/skill/update' && req.method === 'POST') {
    try {
      ...
      const skillPath = path.join(getSafeRepoPath(repo), '.zaf-skills', filename);
