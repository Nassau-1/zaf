...
function safeResolveRepoRoot(userInput) {
...
function composeSeedPrompt(opts, ticketBody) {
  ...
  const repoRoot = safeResolveRepoRoot(repoName);
...
function spawnAgent(opts) {
  ...
  const repoRoot = safeResolveRepoRoot(repoSlug);
  if (!repoRoot) throw new Error('Invalid repo path');
...
`;
  ...
  if (pathname === '/api/repo/skill/update' && req.method === 'POST') {
    try {
      ...
      const repoRoot = safeResolveRepoRoot(repo);
      if (!repoRoot) return send(res, 400, { error: 'Invalid repo path' });
