...
// Performance optimization: Using a shared results array passed by reference avoids O(N²) array allocations during recursive directory traversal.
function walkDir(dirPath, maxDepth, results = [], depth = 0) {
  ...
  for (const e of entries) {
    ...
    if (e.isDirectory()) {
      walkDir(full, maxDepth, results, depth + 1);
    } else if (e.isFile()) {
      results.push(full);
    }
  }
