...
`;
  ...
  if (decodedPathname.indexOf('\0') !== -1) { res.writeHead(400); res.end('Bad Request'); return; }

  let filePath = path.resolve(path.join(STATIC_DIR, decodedPathname === '/' ? 'index.html' : decodedPathname));
  const normalizedStaticDir = path.resolve(STATIC_DIR) + path.sep;
  // If the requested path exactly matches STATIC_DIR, it's also forbidden, but our index.html fallback prevents that.
  if (!filePath.startsWith(normalizedStaticDir) && filePath !== path.resolve(STATIC_DIR)) {
