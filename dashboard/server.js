...
`;
  ...
  if (pathname === '/api/config/github' && req.method === 'POST') {
    try {
      ...
      if (name) execFileSync('git', ['config', '--global', 'user.name', name.replace(/['"]/g, '')], { timeout: 5000 });
      if (email) execFileSync('git', ['config', '--global', 'user.email', email.replace(/['"]/g, '')], { timeout: 5000 });
      const conf = readConfig() || {};
