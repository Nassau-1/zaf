...
// ─── Security helper: resolve repo safely (TKT-SEC-001) ───────────
function resolveRepoRoot(repoSlug) {
  if (repoSlug == null) return null;
  const target = require('path').resolve(REPOS_ROOT, repoSlug);
  const prefix = REPOS_ROOT.endsWith(require('path').sep) ? REPOS_ROOT : REPOS_ROOT + require('path').sep;
  if (!target.startsWith(prefix) && target !== REPOS_ROOT) {
...
function composeSeedPrompt(opts, ticketBody) {
  ...
  const repoRoot = repoName ? resolveRepoRoot(repoName) : null;
  ...
  let skillsSection = '';
  if (repoName) {
    const repoRoot = resolveRepoRoot(repoName);
...
function spawnAgent(opts) {
  ...
  const repoRoot = resolveRepoRoot(repoSlug);
...
const server = http.createServer(async (req, res) => {
  ...
  if (terminateMatch && req.method === 'POST') {
    ...
    try {
      ...
      const today = new Date().toISOString().slice(0, 10);
      const ticketPath = path.join(resolveRepoRoot(entry.meta.repoId || 'zaf'), 'WIP', 'tickets', 'ACTIVE', `${entry.meta.ticketId}.md`);
      try {
  ...
  if (ticketStatusMatch && req.method === 'POST') {
    try {
      ...
      const activeDir = path.join(resolveRepoRoot(repoSlug), 'WIP', 'tickets', 'ACTIVE');
      ...
      const indexFile = path.join(resolveRepoRoot(repoSlug), 'WIP', 'tickets', 'TICKETS.md');
  ...
  if (ticketArchiveMatch && req.method === 'POST') {
    try {
      ...
      const activeDir  = path.join(resolveRepoRoot(repoSlug), 'WIP', 'tickets', 'ACTIVE');
      const archivedDir = path.join(resolveRepoRoot(repoSlug), 'WIP', 'tickets', 'ARCHIVED');
      const ticketPath = path.join(activeDir, `${ticketId}.md`);
      if (!fs.existsSync(ticketPath)) return send(res, 404, { error: 'ticket not found in ACTIVE/' });
...
`;
  ...
  if (pathname === '/api/repo/create' && req.method === 'POST') {
    try {
      ...
      if (mode === 'cli-scaffold' && agentRole && agentHarness) {
        const activeDir = path.join(resolveRepoRoot('zaf'), 'WIP', 'tickets', 'ACTIVE');
        if (!fs.existsSync(activeDir)) fs.mkdirSync(activeDir, { recursive: true });
  ...
  if (pathname === '/api/repo/skills' && req.method === 'GET') {
    ...
    const skillsDir = path.join(resolveRepoRoot(repoSlug), '.zaf-skills');
    try {
  ...
  if (pathname === '/api/repo/skill/update' && req.method === 'POST') {
    try {
      ...
      const skillPath = path.join(resolveRepoRoot(repo), '.zaf-skills', filename);
