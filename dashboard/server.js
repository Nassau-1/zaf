...
function safeResolveRepo(repoSlug) {
  if (!repoSlug) return REPOS_ROOT;
  const resolvedPath = path.resolve(REPOS_ROOT, repoSlug);
...
function spawnAgent(opts) {
  ...
  if (!repoRoot) return { processId: 'ERR', status: 'failed', message: 'Invalid path' };
...
const server = http.createServer(async (req, res) => {
  ...
  if (terminateMatch && req.method === 'POST') {
    ...
    try {
      ...
      if (!repoRoot) throw new Error('Invalid path');
      const ticketPath = path.join(repoRoot, 'WIP', 'tickets', 'ACTIVE', `${entry.meta.ticketId}.md`);
      try {
  ...
  if (ticketStatusMatch && req.method === 'POST') {
    try {
      ...
      const repoRoot = safeResolveRepo(repoSlug);
      if (!repoRoot) return send(res, 400, { error: 'Invalid path' });
  ...
  if (pathname === '/api/repo/create' && req.method === 'POST') {
    try {
      ...
      if (mode === 'cli-scaffold' && agentRole && agentHarness) {
        const zafRoot = safeResolveRepo('zaf');
        if (!zafRoot) throw new Error('Invalid path for zaf');
        const activeDir = path.join(zafRoot, 'WIP', 'tickets', 'ACTIVE');
        if (!fs.existsSync(activeDir)) fs.mkdirSync(activeDir, { recursive: true });
