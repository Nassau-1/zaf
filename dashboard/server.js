...
const server = http.createServer(async (req, res) => {
  ...
  if (pathname === '/api/process/skills' && req.method === 'GET') {
    ...
    const candidates = [];
    const seenCandidateSigs = new Set(); // Performance optimization: O(1) lookup instead of Array.find() on expanding array
    for (let len = 5; len >= 3; len--) {
      ...
      for (const [sig, count] of sigCounts.entries()) {
        if (count >= 2) {
          if (!seenCandidateSigs.has(sig)) {
            seenCandidateSigs.add(sig);
