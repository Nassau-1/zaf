...
function safeResolveRepo(repoSlug) {
  ...
  if (!resolved.startsWith(rootStr) && resolved !== REPOS_ROOT) {
    return null;
  }
  return resolved;
}
...
function composeSeedPrompt(opts, ticketBody) {
  ...
  const repoRoot = repoName ? safeResolveRepo(repoName) : null;
  if (repoName && !repoRoot) return '# Error: Invalid repoName (Path traversal detected)\n';
...
function spawnAgent(opts) {
  ...
  if (!repoRoot) { console.error(`Invalid repoSlug: ${repoSlug}`); return; }
...
const server = http.createServer(async (req, res) => {
  ...
  if (pathname === '/api/skill/save' && req.method === 'POST') {
    try {
      ...
      const repoRoot = safeResolveRepo(repoName || '');
      if (!repoRoot) return send(res, 400, { error: 'invalid repo name' });
  ...
  if (terminateMatch && req.method === 'POST') {
    ...
    try {
      ...
      const today = new Date().toISOString().slice(0, 10);
      const repoRoot = safeResolveRepo(entry.meta.repoId || 'zaf');
      const ticketPath = repoRoot ? path.join(repoRoot, 'WIP', 'tickets', 'ACTIVE', `${entry.meta.ticketId}.md`) : null;
      if (!repoRoot) throw new Error('invalid repo name');
      try {
  ...
  if (ticketStatusMatch && req.method === 'POST') {
    try {
      ...
      const repoRoot = safeResolveRepo(repoSlug);
      if (!repoRoot) return send(res, 400, { error: 'invalid repo name' });
      const activeDir = path.join(repoRoot, 'WIP', 'tickets', 'ACTIVE');
      ...
      const indexFile = path.join(repoRoot, 'WIP', 'tickets', 'TICKETS.md');
  ...
  if (ticketArchiveMatch && req.method === 'POST') {
    try {
      ...
      if (!repoRoot) return send(res, 400, { error: 'invalid repo name' });
      const activeDir  = path.join(repoRoot, 'WIP', 'tickets', 'ACTIVE');
      const archivedDir = path.join(repoRoot, 'WIP', 'tickets', 'ARCHIVED');
      const ticketPath = path.join(activeDir, `${ticketId}.md`);
      if (!fs.existsSync(ticketPath)) return send(res, 404, { error: 'ticket not found in ACTIVE/' });
  ...
  if (pathname === '/api/repo/create' && req.method === 'POST') {
    try {
      ...
      if (mode === 'cli-scaffold' && agentRole && agentHarness) {
        const repoRoot = safeResolveRepo('zaf');
        if (!repoRoot) return;
        const activeDir = path.join(repoRoot, 'WIP', 'tickets', 'ACTIVE');
        if (!fs.existsSync(activeDir)) fs.mkdirSync(activeDir, { recursive: true });
