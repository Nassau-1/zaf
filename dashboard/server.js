...
function getSafeRepoPath(repoSlug) {
  const resolved = path.resolve(REPOS_ROOT, repoSlug || '');
  if ((repoSlug || '').includes('..')) return null;
...
function composeSeedPrompt(opts, ticketBody) {
  ...
  const repoRoot = repoName ? getSafeRepoPath(repoName) : null;
  ...
  let skillsSection = '';
  if (repoName) {
    const repoRoot = getSafeRepoPath(repoName);
    if (!repoRoot) return '';
...
function spawnAgent(opts) {
  ...
  const repoRoot = getSafeRepoPath(repoSlug);
  if (!repoRoot) return { message: 'Invalid repo path' };
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    res.end(JSON.stringify({ error: 'malformed URI encoding' }));
    return;
  }
  if (pathname.indexOf('\0') !== -1 || pathname.indexOf('%00') !== -1) {
    res.writeHead(400, { 'Content-Type': 'application/json' });
    res.end(JSON.stringify({ error: 'poison null byte detected' }));
    return;
  }
  ...
  if (terminateMatch && req.method === 'POST') {
    ...
    try {
      ...
      const today = new Date().toISOString().slice(0, 10);
      const safeRepoRoot_entry = getSafeRepoPath(entry.meta.repoId || 'zaf');
      if (!safeRepoRoot_entry) return send(res, 400, { error: 'Invalid repo path' });
      const ticketPath = path.join(safeRepoRoot_entry, 'WIP', 'tickets', 'ACTIVE', `${entry.meta.ticketId}.md`);
      try {
  ...
  if (ticketStatusMatch && req.method === 'POST') {
    try {
      ...
      const safeRepoRoot_active = getSafeRepoPath(repoSlug);
      if (!safeRepoRoot_active) return send(res, 400, { error: 'Invalid repo path' });
      const activeDir = path.join(safeRepoRoot_active, 'WIP', 'tickets', 'ACTIVE');
      ...
      const safeRepoRoot_index = getSafeRepoPath(repoSlug);
      if (!safeRepoRoot_index) return send(res, 400, { error: 'Invalid repo path' });
      const indexFile = path.join(safeRepoRoot_index, 'WIP', 'tickets', 'TICKETS.md');
  ...
  if (ticketArchiveMatch && req.method === 'POST') {
    try {
      ...
      const safeRepoRoot_active_arch = getSafeRepoPath(repoSlug);
      if (!safeRepoRoot_active_arch) return send(res, 400, { error: 'Invalid repo path' });
      const activeDir  = path.join(safeRepoRoot_active_arch, 'WIP', 'tickets', 'ACTIVE');
      const safeRepoRoot_archived = getSafeRepoPath(repoSlug);
      if (!safeRepoRoot_archived) return send(res, 400, { error: 'Invalid repo path' });
      const archivedDir = path.join(safeRepoRoot_archived, 'WIP', 'tickets', 'ARCHIVED');
      const ticketPath = path.join(activeDir, `${ticketId}.md`);
      if (!fs.existsSync(ticketPath)) return send(res, 404, { error: 'ticket not found in ACTIVE/' });
...
`;
  ...
  if (pathname === '/api/repo/skills' && req.method === 'GET') {
    ...
    try {
      ...
      const skillFiles = files.filter(f => f.endsWith('.zaf-skill.md'));
      const skills = await Promise.all(skillFiles.map(async f => {
  ...
  if (pathname === '/api/repo/skill/update' && req.method === 'POST') {
    try {
      ...
      const repoRoot = getSafeRepoPath(repo);
      if (!repoRoot) return send(res, 400, { error: 'Invalid repo path' });
  ...
  if (!filePath.startsWith(STATIC_DIR + path.sep) && filePath !== STATIC_DIR) { res.writeHead(403); res.end('Forbidden'); return; }
